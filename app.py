...
Includes homepage, resume, projects, prediction, and model interpretability pages
"""
...
# Import model interpretability module
from src.model_interpretability import explain_prediction

def log_env_info():
    """Print Python version and package info for debugging"""
    try:
...
# We load these once at startup to avoid reloading on every prediction
models = {}
scaler = None
imputer = None
X_train = None  # Store training data for interpretability
...
def load_models():
    ...
    We check if files exist because XGBoost might not be available
    if libomp isn't installed on the system.
    """
    global models, scaler, imputer, X_train
    
    try:
        # First, load the preprocessors !
        ...
        # Load training data for SHAP/LIME explanations
        try:
            X_train = pd.read_csv('data/X_train.csv')
            print(f"Training data loaded for interpretability: {X_train.shape}")
        except Exception as train_err:
            print(f"Warning: Could not load training data for interpretability: {train_err}")
            X_train = None
...
def interpretability():
    """Model interpretability page route"""
    return render_template('interpretability.html')
...
def explain():
    ...
    try:
        ...
        # Use XGBoost for explanation
        best_model_name = 'XGBoost' if 'XGBoost' in models else list(models.keys())[0] if models else None
        
        if best_model_name is None or X_train is None:
            return jsonify({'error': 'Model or training data not available for explanation'}), 400
        ...
        explanations = explain_prediction(
            ...
            X_instance=input_scaled.values,
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'error': f'Explanation generation failed: {str(e)}'}), 400
